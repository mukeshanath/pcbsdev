...
defined('__ROOT__') OR exit('No direct script access allowed');
 ...
 include 'db.php'; 
      ...
      class cnotesales extends My_controller
      {
          public function index()
          {
              $this->view->title = __SITE_NAME__ . ' - cnotesales';
              $this->view->render('cnote/cnotesales');
          }
      }
       ...
       if(isset($_POST['cust_code'])) 
            {
                ...
                $soid = $db->query("SELECT TOP 1 salesorderid FROM cnotesales ORDER BY cast(dbo.GetNumericValue(salesorderid) AS decimal) DESC");
                if($soid->rowCount()==0){
               ...
               $sql = "INSERT INTO cnotesales (stncode,stnname,br_code,br_name,salesorderid,cust_code,cust_name,cnote_serial,cnote_count,cnote_start,cnote_status,cnote_procured,cnote_charge,CNote_Stationary_Charge,GST,GrandAmt,NetAmt,pMode,RcptNo,date_added,user_name,flag) 
                                       VALUES ('$stncode','$stnname','$br_code','$br_name','$salesorderid','$cust_code','$cust_name','$cnote_serial','$cnote_count','$cnote_start','Allotted','$cnote_procured','$cnote_amount','$cnote_amount','$gst','$grnd_amt','$net_amt','$payment_type','$chqdep_no','$date_added','$username',1)";
               $result  = $db->query($sql);
                ...
                for($i = 0; $i < count($_POST['salespono']); $i++)
                {
                    ...
                    $allotted = "UPDATE cnotenumber SET cnote_status ='Allotted', br_code='$br_code', br_name='$br_name',cust_code='$cust_code',cust_name='$cust_name',allot_date='$date_added', allot_po_no='$salesorderid' WHERE cnote_status='Printed' AND cnote_station='$stncode' AND cnote_number BETWEEN '$salesstart' AND '$salesend' AND cnote_serial='$cnote_serial'";
           ...
           header("location: cnotesaleslist?success=1");
