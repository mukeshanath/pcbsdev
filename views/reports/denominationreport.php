...
<!-- Bread Crumb -->
                                ...
                                <div class="au-breadcrumb-content">
                                    <div class="au-breadcrumb-left">
                                        ...
                                        <ul class="list-unstyled list-inline au-breadcrumb__list">
                                            ...
                                            <li class="list-inline-item">Denomination Report</li>
                                        </ul>
                                    </div>
 ...
 <div class="boxed boxBt panel panel-primary">
                    <!-- Tittle -->
    <div class="title_1 panel-heading"><p style="font-weight:600;font-size:20px;color:white;margin-top:6px;">Denomination Report</p>
   ...
   <div class="tab_form" >
   <input type="text" id="stncode" class="hidden" name="den_stncode" value="<?=$stationcode;?>">
    ...
    <div class="tab-content">
     ...
     <table cellpadding="0" border="0" cellspacing="0" class="table" style="width:100%">
	   <tr><td class="filter-title">Report Name</td></tr>
        <tr>		
            <td>
            <select class="form-control" id="den_reporttype" name="den_reporttype">
            <option value="">Select Type</option>     
            <option value="denomination">Denomination Report</option>     
            </select>
            </td>
       </tr>
    </table>
     ...
     <table cellpadding="0" border="0" cellspacing="0" class="table" style="width:100%">
	   <tr><td class="filter-title" colspan="3">Date Range</td></tr>
        <tr>		
            <td><input type="date" class="form-control today" name="den_date_from" id="den_date_from"></td>
            <td>to</td>
              
            <td><input type="date" class="form-control today" name="den_date_to" id="den_date_to"></td>
            <td>
     ...
     <div class="report-header" style="border-bottom:2px solid black;margin-bottom:10px">
          <div class="date-between-column" style="text-align:left;">
               <h5 class="preview-heading">Preview of Denomination Summary between <span id="head-date-from"></span> and <span id="head-date-to"></span></h5>
          </div>
     </div>
     ...
     <div class="title_1 title panel-heading" style="text-align:left;">
          ...
          <div class="dt-buttons"> 
               <button class="dt-button buttons-excel buttons-html5" aria-controls="data-table" type="submit"><span><img data-toggle="tooltip" title="Export to EXCEL" style="width:100%;" src="https://img.icons8.com/color/48/000000/ms-excel.png"></span></button> 
     ...
     <table class="data-table invoice-report-table" id="data-table" cellpadding="0" border="1" cellspacing="0" class="table" style="width:100%;">
          <thead>
               <tr>
                    <th></th>
               </tr>
...
<script>
     $('#filter-next').click(function(){
          if($('#den_reporttype').val().trim()==''){
               swal("Please Select Report Type");
          }
          else{
               progress();
          var den_reporttype   = $('#den_reporttype').val();
          var den_date_from     = $('#den_date_from').val();
          var den_date_to      = $('#den_date_to').val();
          var den_stncode      = $('#stncode').val();
     
          $.ajax({
            url:"<?php echo __ROOT__ ?>reportcontroller",
            method:"POST",
            data:{den_reporttype,den_date_from,den_date_to,den_stncode},
            dataType:"text",
            success:function(data)
            {
               stopprogress();
            $('.invoice-report-table').html(data);
            $('.preview-heading').html('Rate Denomination Summary Between '+den_date_from+' and '+den_date_to);
            $('#report-title').html('Denomination Summary Report as on '+new Date().toISOString().split('T')[0]);
