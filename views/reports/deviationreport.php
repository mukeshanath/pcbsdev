...
<!-- Bread Crumb -->
                            ...
                            <div class="col-md-12">
                                <div class="au-breadcrumb-content">
                                    <div class="au-breadcrumb-left">
                                        ...
                                        <a href="<?php echo __ROOT__ . 'deviationreport'; ?>">Deviation Report Filters</a>
                                      </li>
                                        </ul>
                                    </div>
 ...
 <div class="boxed boxBt panel panel-primary">
                    <!-- Tittle -->
    <div class="title_1 panel-heading"><p style="font-weight:600;font-size:20px;color:white;margin-top:6px;">Deviation Report</p>
   ...
   <div class="tab_form" >
   <input type="text" id="stncode" class="hidden" name="dev_stncode" value="<?=$stationcode;?>">
    ...
    <div class="tab-content">
     ...
     <table cellpadding="0" border="0" cellspacing="0" class="table" style="width:100%">
	   <tr><td class="filter-title">Report Name</td></tr>
        <tr>		
               <td><select class="form-control" id="dev_reporttype" name="dev_reporttype">
            <option value="">Select Type</option>     
            <option value="weightdeviation">Credit Rate Deviation Report</option>     
            <option value="cashdeviation">Cash Rate Deviation Report</option>     
            <!-- <option value="actuelvsvolume">Actual vs Volumetric Report</option>      -->
            </select>
            </td>
       </tr>
    </table>
     ...
     <table cellpadding="0" border="0" cellspacing="0" class="table" style="width:100%">
	   <tr><td class="filter-title" colspan="3">Date Range</td></tr>
        <tr>		
            <td><input type="date" class="form-control today" name="dev_date_from" id="dev_date_from"></td>
            <td>to</td>
              
            <td><input type="date" class="form-control today" name="dev_date_to" id="dev_date_to"></td>
            <td>
     ...
     <div class="date-filter col-sm-4">
     <table cellpadding="0" border="0" cellspacing="0" class="table" style="width:100%">
	   <tr><td class="filter-title" colspan="3">Deviation Filter</td></tr>
        <tr>		
            
            <td>
           <select name="dev_filter" id="dev_filter">
                <option value="ALL">ALL</option>
                <option value="BRLTOR">BillRate &lt; OpRate</option>
                <option value="ORLTBR">OpRate &lt; BillRate</option>
           </select>
            </td>
       </tr>
    </table>
     ...
     <div class="report-header" style="border-bottom:2px solid black;margin-bottom:10px">
          <div class="date-between-column" style="text-align:left;">
               <h5 class="preview-heading">Preview of Deviation Summary between <span id="head-date-from"></span> and <span id="head-date-to"></span></h5>
          </div>
     </div>
...
<script>
     $('#filter-next').click(function(){
          if($('#dev_reporttype').val().trim()==''){
               swal("Please Select Report Type");
          }
          else{
               progress();
          var dev_reporttype   = $('#dev_reporttype').val();
          var dev_date_from     = $('#dev_date_from').val();
          var dev_date_to      = $('#dev_date_to').val();
          var dev_filter      = $('#dev_filter').val();
          var dev_stncode      = $('#stncode').val();
               // alert(dev_filter);
          $.ajax({
            url:"<?php echo __ROOT__ ?>reportcontroller",
            method:"POST",
            data:{dev_reporttype,dev_date_from,dev_date_to,dev_stncode,dev_filter},
            dataType:"text",
            ...
            $('.preview-heading').html('Credit Rate Deviation Report Between '+dev_date_from+' and '+dev_date_to);
            $('#report-title').html('Deviation  Report as on '+new Date().toISOString().split('T')[0]);
            }
            if(dev_reporttype=='cashdeviation'){
            $('.preview-heading').html('Cash Rate Deviation Report Between '+dev_date_from+' and '+dev_date_to);
            $('#report-title').html('Deviation  Report as on '+new Date().toISOString().split('T')[0]);
            }

          //   else if(reporttype=='invoicesummarywithcoll'){
          //   $('.preview-heading').html('Deviation Summary with Collection Between '+dev_date_from+' and '+dev_date_to);
          //   $('#report-title').html('Deviation Summary Report as on '+new Date().toISOString().split('T')[0]);
          //   }
          //   else if(reporttype=='monthwiseinvoicesummary'){
          //   $('.preview-heading').html('Month wise Invoice Summary Between '+months[parseInt(dev_date_from)]+' and '+months[parseInt(dev_date_to)]);
          //   $('#report-title').html('Month wise Invoice Summary Between '+months[parseInt(dev_date_from)]+' and '+months[parseInt(dev_date_to)]);
          //   }
            }
            });
          }
...
function exportexcel(type, fn, dl){
     //data-table
     var sel = document.getElementById("dev_reporttype");//report type option             
