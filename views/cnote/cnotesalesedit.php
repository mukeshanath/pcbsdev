...
if(isset($_GET['salesid'])){
    ...
    $salesdata = $db->query("SELECT * FROM cnotesales WHERE salesorderid='$salesorderid' AND stncode='$stationcode'")->fetch(PDO::FETCH_OBJ);
}
...
<div class="au-breadcrumb m-t-75" style="margin-bottom: -5px;">
            ...
            <div class="alert-primary" role="alert">
                ...
                "<div class='alert alert-success alert-dismissible'>
                                ...
                                Cnote Sales updated successfully.
                </div>";
                }
                ...
                <form method="POST" autocomplete="off" action="<?php echo __ROOT__ ?>cnotesalesedit" accept-charset="UTF-8" class="form-horizontal" id="formcnotesales" enctype="multipart/form-data">
        ...
        <input id="br_code" name="br_code" class="form-control input-sm br_code" type="text" readonly value="<?=$salesdata->br_code?>">
        <p id="brcode_validation" class="form_valid_strings">Enter Valid Branch Code</p>
        </div>
        </td>
        <td>Branch Name</td>
        <td><div class="col-sm-12">
             <input id="br_name" name="br_name" value="<?=$salesdata->br_name?>" readonly class="form-control input-sm"  type="text"></div>
             </div>
        </td>	
        </tr>
        <!-- Second Row	 -->
        ...
        <div class="autocomplete col-sm-12 ">
              <input class="form-control input-sm cust_code"  name="cust_code" type="text" id="cust_code" onkeyup="verifycustomer();fetchsalesnote()" value="<?=$salesdata->cust_code?>">
              <p id="custcode_validation" class="form_valid_strings">Enter Valid Customer Code</p>
              </div>
        </td>	
        <td>Customer Name</td>
        <td><div class="autocomplete col-sm-12">
                <input id="cust_name" name="cust_name" type="text" value="<?=$salesdata->cust_name?>" class="form-control input-sm">
            </div>
        </td>
        <td>Serial Type</td>
        <td>
        <input id="cnote_serial" name="cnote_serial" type="text" readonly value="<?=$salesdata->cnote_serial?>" class="form-control input-sm">
        </td>	
        <td>Quantity</td>
        <td>
            <input  class="form-control input-sm cnote_count" name="cnote_count" type="text" readonly id="cnote_count" value="<?=$salesdata->cnote_count?>">
        ...
        <tr> 
            ...
            <td></td>
            <td></td>
            <td> <button type="button" class="btn btn-primary btnattr_cmn" id="updatesales" name="cnotesalesupdate" onclick="verify()">
		...
		<table class="table">
			<tbody>
				...
				<tr>
					...
					<td><input tabindex="-1" readonly class="form-control input-sm "  name="cnote_amount" type="text" id="cnote_amount" value="<?=$salesdata->CNote_Stationary_Charge?>"> </td>
					</tr></tr>
				...
				<tr>
					<td>Grand Amount</td>
					<td>
                       <input tabindex="-1" readonly class="form-control border-form" name="grnd_amt" type="text" id="grnd_amt" value="<?=$salesdata->GrandAmt?>">
                    </td>
                    </tr></tr>
                    <td>Total GST</td>
					<td>
                      <input tabindex="-1" readonly class="form-control border-form" name="gst" type="text" id="gst" value="<?=$salesdata->GST?>">
                    </td>
				</tr>
				...
				<tr>
					<td>Net Amount</td>
					<td>
                     <input tabindex="-1" readonly class="form-control border-form" name="net_amt" type="text" id="net_amt" value="<?=$salesdata->NetAmt?>">
                    </td>
                    <td></td>
				</tr>
                <tr>
...
<script>
    function verify()
    {
        ...
        {
           swal("Entered valid customer code !!");
        }
        else{
            $('#updatesales').attr('type','submit');
            $('#updatesales').attr('name','cnotesalesupdate');
        }
    }
...
<script>
  var customercode =[<?php  
    $sql=$db->query("SELECT cust_code FROM customer WHERE comp_code='$stationcode' AND br_code ='$salesdata->br_code'");
 ...
 $brname=$db->query("SELECT cust_name FROM customer WHERE comp_code='$stationcode' AND br_code ='$salesdata->br_code'");
 while($rowbrname=$brname->fetch(PDO::FETCH_ASSOC))
 {
 echo "'".$rowbrname['cust_name']."',";
 }
 ?>]

autocomplete(document.getElementById("cust_code"), customercode); 
autocomplete(document.getElementById("cust_name"), customername); 
