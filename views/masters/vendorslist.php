...
<div class="au-breadcrumb m-t-75" style="margin-bottom: -5px;">
      ...
      <div class="boxed boxBt panel panel-primary">
        <div class="title_1 title panel-heading">
              <p style="font-weight:600;font-size:20px;color:white;margin-top:6px;">Vendor Filters</p>
         <div class="align-right" style="margin-top:-35px;">

         <a style="color:white" href="<?php echo __ROOT__ ?>vendors"><button type="button" class="btn btn-success btnattr" >
      ...
      <div class="content panel-body">
       ...
       <input placeholder="" class="form-control border-form" name="user_name" type="hidden" id="user_name" value="<?php echo $_SESSION['user_name']; ?>" style="display:none;">    
          <table class="table" cellspacing="0" cellpadding="0" align="center">	
            ...
            <td>Vendor Code</td>
            <td>
            <input class="form-control br_code" name="vendor_code" type="text" value="<?php if(isset($_POST['vendor_code'])){ echo  $_POST['vendor_code']; } ?>" id="vendor_code">
            </td>
                
            <td>Vendor Name</td>
            <td>
                <input class="form-control border-form" name="vendor_name" type="text" value="<?php if(isset($_POST['vendor_name'])){ echo  $_POST['vendor_name']; } ?>" id="vendor_name" >
            </td>
            <!-- <td>Phone</td>
            <td>
            <input class="form-control border-form" name="vendor_phone" type="text" value="<?php if(isset($_POST['vendor_phone'])){ echo  $_POST['vendor_phone']; } ?>" id="vendor_phone">
            </td> -->
            <td>Active</td>
            <td>
...
<div class="col-sm-12 data-table-div" >
          <div class="title_1 title panel-heading">
              <p style="font-weight:600;font-size:20px;color:white;margin-top:10px;">Vendors List</p>
       ...
       <table id="data-table" class="table data-table" cellspacing="0" cellpadding="0" align="center">
         ...
         <th class="center">
                    ...
                    <th class="hidden">Current Pono</th>
                    <th>Active</th>
                    <th>Actions</th>
              </tr>
                </thead>
              ...
              <?php 
                      ...
                      {    
                           ...
                           if(! empty($vendor_code)) {
                              $conditions[] = "vendor_code LIKE '$vendor_code%'";
                            } 
                            if(! empty($vendor_name)) {
                              $conditions[] = "vendor_name LIKE '$vendor_name%'";
                            }
                            if(! empty($vendor_phone)) {
                              $conditions[] = "vendor_phone LIKE '$vendor_phone%'";
                            }
                            if(! empty($vendor_status)) {
                              $conditions[] = "vendor_status='$vendor_status'";
                            }
                            ...
                            if (count($conditions) > 0) {
                                $query = $db->query("SELECT * FROM vendor  WHERE  $conditons"); 
                            }
                            if (count($conditions) == 0) {
                              $query = $db->query("SELECT * FROM vendor $stn_condition ORDER BY vend_id DESC"); 
                          }
                      }
                      //Normal Station list
                      else{
                      ...
                      if($query->rowcount()==0)
                      {
                       ...
                       while ($row = $query->fetch(PDO::FETCH_OBJ))
     
                          {
                            ...
                            echo "<td>".$row->vendor_mobile."</td>";
                            echo "<td>".$row->vendor_email."</td>";
                            echo "<td>".$row->vendor_contactperson."</td>";                          
                            echo "<td class='hidden'>".$row->stnname."</td>";                          
                            echo "<td class='hidden'>".$row->vendor_add."</td>";                          
                            echo "<td class='hidden'>".$row->vendor_city."</td>";                          
                            echo "<td class='hidden'>".$row->vendor_state."</td>";                          
                            echo "<td class='hidden'>".$row->vendor_country."</td>";                          
                            echo "<td class='hidden'>".$row->vendor_phone."</td>";                          
                            echo "<td class='hidden'>".$row->vendor_fax."</td>";                          
                            echo "<td class='hidden'>".$row->vendor_remarks."</td>";                          
                            echo "<td class='hidden'>".$row->curpono."</td>";                          
                            echo "<td>".$row->vendor_status."</td>"; 
                            echo "<td><div class='hidden-sm hidden-xs action-buttons'>
                            <a  class='btn btn-primary btn-minier' data-rel='tooltip' title='View' href='vendorsdetail?vend_id=".$row->vend_id."'>
                                  <i class='ace-icon fa fa-eye bigger-130'></i>
                                </a>
                            <a class='btn btn-success btn-minier' data-rel='tooltip' title='Edit' href='vendorsedit?vend_id=".$row->vend_id."'>
                                  <i class='ace-icon fa fa-pencil bigger-130'></i>
                               </a>

                                 <a  class='btn btn-danger btn-minier bootbox-confirm' id='vendor,$row->vend_id,vend_id' onclick='".delete($db,$user)."'>
