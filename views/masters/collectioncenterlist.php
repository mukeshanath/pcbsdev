...
<div class="au-breadcrumb m-t-75" style="margin-bottom: -5px;">
                            ...
                            <div class="au-breadcrumb-content">
                                <div class="au-breadcrumb-left">
                                    ...
                                    <ul class="list-unstyled list-inline au-breadcrumb__list">
                                        ...
                                        <li class="list-inline-item">Collection Center List</li>
                                    </ul>
                                </div>
        ...
        <div class="alert-primary" role="alert">
               ...
               <?php 
                  ...
                  "<div class='alert alert-success alert-dismissible'>
                                  <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
                                  <h4><i class='icon fa fa-check'></i> Alert!</h4>
                                  Collection Center Successfully.
                                </div>";
                  }
      ...
      <div class="boxed boxBt panel panel-primary">
        <div class="title_1 title panel-heading">
              <p style="font-weight:600;font-size:20px;color:white;margin-top:6px;">Collection Center Filters</p>
         <div class="align-right" style="margin-top:-35px;">

         <a style="color:white" href="<?php echo __ROOT__ ?>collectioncenter"><button type="button" class="btn btn-success btnattr" >
      ...
      <div class="content panel-body">
       ...
       <input placeholder="" class="form-control border-form" name="user_name" type="hidden" id="user_name" value="<?php echo $_SESSION['user_name']; ?>" style="display:none;">    
          <table class="table" cellspacing="0" cellpadding="0" align="center">	
            ...
            <td>Center Code</td>
            <td>
            <input class="form-controle"  type="text" name="cc_code" id="cc_code" value="<?php if(isset($_POST['cc_code'])){ echo $_POST['cc_code']; } ?>">          
            </td>                
            <td>Center Name</td>
            <td>
                <input class="form-control border-form"  type="text" name="cc_name" id="cc_name"  value="<?php if(isset($_POST['cc_name'])){ echo $_POST['cc_name']; } ?>">               
            </td>            
            <td>Active</td>
            <td>
            <select class="form-control border-form" name="cc_active" id="cc_active">                
...
<div class="col-sm-12 data-table-div">
  ...
  <div class="title_1 title panel-heading">
              <p style="font-weight:600;font-size:20px;color:white;margin-top:6px;">Collection Center List</p>
        ...
        <thead><tr class="form_cat_head">
            <th class="center">
              ...
              <th>Coll Cent code</th>
              <th>Coll Cent Name</th>
              <th>Branch</th>
              <th class="hidden">Station Name</th>
              <th class="hidden">Addr Type</th>
              ...
              <tbody>
                <?php  //Filter Button Event
                     ...
                     $conditions[] = "stncode='$stationcode'";
                      ...
                      //branch filter conditions
                      //collectioncenter filter conditions
                      $user_cc = $datafetch['cust_code'];
                      if(!empty($user_cc)){
                      $conditions[] = "cc_code='$user_cc'";
                      }
                      //collectioncenter filter conditions
                     ...
                     {    
                          ...
                          if(! empty($cc_code)) {
                            $conditions[] = "cc_code LIKE '$cc_code%'";
                          }
                          if(! empty($cc_name)) {
                            $conditions[] = "cc_name LIKE '$cc_name%'";
                          }
                           if(! empty($cc_active)) {
                             $conditions[] = "cc_active='$cc_active'";
                           }
                           ...
                           if (count($conditions) > 0) {
                               $query = $db->query("SELECT * FROM collectioncenter  WHERE  $conditons"); 
                           }
                           if (count($conditions) == 0) {
                             $query = $db->query("SELECT * FROM collectioncenter WHERE  $conditons ORDER BY bkcid DESC"); 
                         }
                     }
                     //Normal Station list
                     else{
                     ...
                     if($query->rowcount()==0)
                     {
                       echo "";
                        ...
                        while ($row = $query->fetch(PDO::FETCH_OBJ))
                        {
                          ...
                          echo "<td class='hidden'>".$row->cc_addrtype."</td>";
                          echo "<td class='hidden'>".$row->cc_addr."</td>";
                          echo "<td>".$row->cc_phone."</td>";
                          echo "<td>".$row->cc_email."</td>";                           
                          echo "<td class='hidden'>".$row->cc_fax."</td>";                           
                          echo "<td class='hidden'>".$row->cc_remarks."</td>";                           
                          echo "<td class='hidden'>".$row->cc_tin."</td>";                           
                          echo "<td class='hidden'>".$row->cc_gstin."</td>";                           
                          echo "<td class='hidden'>".$row->cc_fsc."</td>";                           
                          echo "<td class='hidden'>".$row->cnote_charges."</td>";                           
                          echo "<td class='hidden'>".$row->spl_discount."</td>";                           
                          echo "<td class='hidden'>".$row->cc_deposit."</td>";                           
                          ...
                          echo "<td>".$row->cc_active."</td>";
                          echo "<td><div class='hidden-sm hidden-xs action-buttons'>
                              <a  class='btn btn-primary btn-minier'  data-rel='tooltip' title='View' href='collectioncenterdetail?ccid=".$row->bkcid."'  >
                                <i class='ace-icon fa fa-eye bigger-130'></i>
                              </a>

                               <a class='btn btn-success btn-minier' data-rel='tooltip' title='Edit' href='collectioncenteredit?ccid=".$row->bkcid."'   >
                                <i class='ace-icon fa fa-pencil bigger-130'></i>
                               </a>

                               <a  class='btn btn-danger btn-minier bootbox-confirm' id='collectioncenter,$row->bkcid' onclick='".delete($db,$user)."'>
