...
<div class="au-breadcrumb m-t-75" style="margin-bottom: -5px;">
         ...
         <a style="color:white" href="<?php echo __ROOT__ ?>bainboundbatch"><button type="button" class="btn btn-success btnattr" >
                  ...
                  <?php 
                             ...
                             {    
                                 ...
                                 if (count($conditions) > 0) {
                                     $query = $db->query("SELECT * FROM bainboundbatch_master  WHERE  $conditons"); 
                                 }
                                 if (count($conditions) == 0) {
                                 $query = $db->query("SELECT * FROM bainboundbatch_master WHERE  $conditons ORDER BY bainboundmasterid DESC"); 
                             }
                             } 
                             else{ 
                              $conditons = implode(' AND ', $conditions);
  
                              $query = $db->query("SELECT * FROM bainboundbatch_master WHERE  $conditons AND date_added BETWEEN DATEADD(DAY,- datepart(dw, getdate()), GETDATE()) AND DATEADD(DAY, 1, GETDATE()) ORDER BY bainboundmasterid DESC");
                             }
                             if($query->rowcount()==0)
                             {
                             echo "";
                         ...
                         while ($row = $query->fetch(PDO::FETCH_OBJ))
                          {
                            ...
                            <a class='btn btn-primary btn-minier' href='bainboundbatchdetail?mfmasno=$row->bainboundmasterid' data-rel='tooltip' title='View'>
                             <i class='ace-icon fa fa-eye bigger-130'></i>
                            </a>
                            <a class='btn btn- btn-minier' href='bainboundbatchsummary?mfmasno=$row->bainboundmasterid'  target='_blank' data-rel='tooltip' title='Edit'>
