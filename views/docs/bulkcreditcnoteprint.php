<?php
if(isset($_GET['array_cno'])){
    $station = $_GET['station'];
    $invno_json = explode(",",$_GET['array_cno']);
    $bulk_cuscode = explode(",",$_GET['custcodes']);
    

    foreach($invno_json as $invno):
        foreach($bulk_cuscode as $cust_codes):
?>
...
$printflags = $db->query("SELECT credit_accounts_copy,credit_pod_copy,credit_consignor_copy FROM customer WHERE cust_code='$custcode' AND comp_code='$stationcode'")->fetch(PDO::FETCH_OBJ);
...
for($i=0;$i<count($challan_bottom);$i++){
     ...
     //get cnote details
     $cashcno = $db->query("SELECT TOP 1 *, FORMAT(date_added, 'yyyy/MM/dd HH:mm') AS time,(select br_address from branch where branch.br_code=credit.br_code AND branch.stncode=credit.comp_code) as branchaddr,(select br_city from branch where branch.br_code=credit.br_code AND branch.stncode=credit.comp_code) as branchcity,(select br_state from branch where branch.br_code=credit.br_code AND branch.stncode=credit.comp_code) as branchstate,(select br_mobile from branch where branch.br_code=credit.br_code AND branch.stncode=credit.comp_code) as branchmobile,(select br_email from branch where branch.br_code=credit.br_code AND branch.stncode=credit.comp_code) as branchmail FROM credit  WHERE cno='$cno' AND comp_code='$stationcode' AND cust_code='$custcode'")->fetch(PDO::FETCH_OBJ);
    
     //get bookcounter details
     $bcounter = $db->query("SELECT * FROM customer LEFT JOIN branch ON customer.br_code = branch.br_code 

     
     
     AND branch.stncode=customer.comp_code WHERE customer.cust_code='$cashcno->cust_code' AND comp_code='$cashcno->comp_code'")->fetch(PDO::FETCH_OBJ);

     //get branch address det
     //get company Details
     $company = $db->query("SELECT * FROM station WHERE stncode='$stationcode'")->fetch(PDO::FETCH_OBJ);
     $br_ncode = $db->query("SELECT * FROM branch WHERE br_code='$cashcno->br_code' AND stncode='$cashcno->comp_code'")->fetch(PDO::FETCH_OBJ);

     
?>


<?php if(!empty($cashcno->cno)) : ?>

<title>Creditchallan-<?=$stationcode.$cno?></title>
<div id="single-cnote">
...
?>
  ...
  if($challan_bottom[$i]=="CREDIT - CONSIGNOR COPY")
   {
    echo '<div class="company-name-logo" style="line-height:3px;text-align: center;line-height:14px;font-size:11px;margin-bottom:6px;">
    <p><b>'.$cashcno->branchaddr.'<br>'.$cashcno->branchcity.'<br>'.$cashcno->branchmobile.'</b>,<b>'.$cashcno->branchmail.'</b></p>
  ...
  ?>
                    ...
                    <?php 
               if($challan_bottom[$i]=="CREDIT - POD COPY")
               {
                    ?>
                    <div class="">
               ...
               <div class="sender-details">
                    ...
                    <p><?=$bcounter->cust_name?></p>
                    <p><?=$bcounter->cust_addr?></p>
                    <p>Mobile : <?=$bcounter->cust_mobile?></p>
                    <p>GSTIN : <?=$bcounter->cust_gstin?></p>
               ...
               if($challan_bottom[$i]=="CREDIT - POD COPY")
               {
                    ...
                    <img style="width:180px;margin-bottom:5px;" src="barcode?text=<?=$stationcode.$cashcno->cno?>&codetype=code128&orientation=horizontal&size=40&print=<?=$stationcode.$cashcno->cno?>" class="barcode"><h3 style="color:black;margin-top:0%;"><?=$stationcode.$cashcno->cno?><?=($decoded_type=='reprinted'?' (Duplicate)':'')?></h3>
                    </div>
               ...
               <div style="height: 90px;padding: 2px;line-height: 8px;">
                    ...
                    <!-- <div class="cnote-messages-right">
                         ...
                         if($challan_bottom[$i]=='CONSIGNOR COPY'){
                              //echo "<h1>Rs. ".round($cashcno->amt);.00."</h1>";
                         }
                         ?>
                         </div>
                    </div> -->
          ...
          <div class="right-corner">
                         ...
                         <div class="br-ncode">
                         <?=$br_ncode->br_ncode?> - <?=$cashcno->br_code?>
                         </div>
                         <div class="dt date-time-pcs">
                          
                              Date : <?=$cashcno->time?> Hrs
                         ...
                         <?php
                       $gst = 0.18;
                       $gstnormal_value = 0.82;

                         ?>
                         <!-- <div class="charges" style="margin-top:5px;">
                              <b><u style="color: black;margin-top:15px;">Amount</u></b>
                              <p>Charges :  <?= $cashcno->amt * $gstnormal_value ?></p>
                              <p>GST : <?= $cashcno->amt * $gst ?></p> 
                              <?php
                         if($challan_bottom[$i]=='CREDIT - ACCOUNTS COPY'){
                             // echo "<p>Charges : ".$cashcno->amt * $gstnormal_value."</p>";
                             // echo "<p>GST : ".$cashcno->amt * $gst."</p>";
                             // echo "<p>Total : ".number_format((float)$cashcno->amt, 2, '.', '')."</p>";
                         }else{
                             // echo "<p>Charges : NA</p>";
                             // echo "<p>GST : NA</p>";
                         }
