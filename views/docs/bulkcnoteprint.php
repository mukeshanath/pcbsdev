...
$print_station = $_GET['print_station'];
     $cnote_json = explode(",",$_GET['cnote_json']);
     foreach($cnote_json as $cnoteno){
        $cashcno = $db->query("SELECT TOP 1 *, FORMAT(date_added, 'yyyy/MM/dd HH:mm') AS time FROM credit  WHERE cno='$cnoteno' AND comp_code='$stationcode' AND cust_code='$print_customer'")->fetch(PDO::FETCH_OBJ);
     }
...
$print_station = $_GET['print_station'];
     $cnote_json = explode(",",$_GET['cnote_json']);
     foreach($cnote_json as $cnoteno){
        $cashcno = $db->query("SELECT * FROM credit  WHERE cno='$cnoteno' AND comp_code='$stationcode' AND cust_code='$print_customer'")->fetch(PDO::FETCH_OBJ);
     }
...
for($i=0;$i<count($challan_bottom);$i++){
     ...
     //get cnote details
     $cashcno = $db->query("SELECT TOP 1 *, FORMAT(date_added, 'yyyy/MM/dd HH:mm') AS time FROM credit  WHERE cno='$cno' AND comp_code='$stationcode' AND cust_code='$print_customer'")->fetch(PDO::FETCH_OBJ);
